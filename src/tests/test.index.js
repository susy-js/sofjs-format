...
describe('test sofjs-format object', () => {
  describe('test formatQuantity', () => {
    ...
    it('test format object SofSyncing', () => {
  ...
  describe('test decode of getBlockByNumber', () => {
    ...
    const formattedData = format.formatOutputs('sof_getBlockByHash', payload);
  ...
  describe('test formatInputs', () => {
    it ('should handle sof_getBalance normally', () => {
      const encodedBalance = format.formatInputs('sof_getBalance', ['0xb60e8dd61c5d32be8058bb8eb970870f07233155']);

      assert.equal(encodedBalance, '0xb60e8dd61c5d32be8058bb8eb970870f07233155');
    });

    it ('should handle sof_getBalance 2 arguments normally', () => {
      const encodedBalance = format.formatInputs('sof_getBalance', ['0xb60e8dd61c5d32be8058bb8eb970870f07233155', 'latest']);

      assert.equal(encodedBalance[0], '0xb60e8dd61c5d32be8058bb8eb970870f07233155');
      assert.equal(encodedBalance[1], 'latest');
    });

    it ('should handle sof_sendTransaction normally', () => {
      const encodedSendTransaction = format.formatInputs('sof_sendTransaction', [{
        'from': '0xb60e8dd61c5d32be8058bb8eb970870f07233155',
        'to': '0xd46e8dd67c5d32be8058bb8eb970870f07244567',
        'gas': new BN('30400'), // 30400,
        'gasPrice': '10000000000000', // 10000000000000
  ...
  describe('test formatOutputs', () => {
    it ('should handle invalid sof_getTransactionByBlockHashAndIndex normally', () => {
      try {
        const decodedHashRate = format.formatInputs('sof_getTransactionByBlockHashAndIndex', ['0x']);
      } catch (error) {
        assert.equal(typeof error, 'object');
      }
    });

    it ('should handle sof_getFilterChanges normally during sof_newBlockFilter', () => {
      const decodedTxHashArray = format.formatOutputs('sof_getFilterChanges', ['0xdf829c5a142f1fccd7d8216c5785ac562ff41e2dcfdf5785ac562ff41e2dcf55', '0xdf829c5a142f1fccd7d8216c5785ac562ff41e2dcfdf5785ac562ff41e2dcf55']);
    ...
    it ('should handle sof_getFilterChanges normally during sof_newPendingTransactionFilter', () => {
      const decodedTxHashArray = format.formatOutputs('sof_getFilterChanges', ['0xdf829c5a142f1fccd7d8216c5785ac562ff41e2dcfdf5785ac562ff41e2dcf55', '0xdf829c5a142f1fccd7d8216c5785ac562ff41e2dcfdf5785ac562ff41e2dcf55']);

      assert.equal(decodedTxHashArray.toString(10), ['0xdf829c5a142f1fccd7d8216c5785ac562ff41e2dcfdf5785ac562ff41e2dcf55', '0xdf829c5a142f1fccd7d8216c5785ac562ff41e2dcfdf5785ac562ff41e2dcf55']);
    });

    it ('should handle sof_hashrate normally', () => {
      const decodedHashRate = format.formatOutputs('sof_hashrate', '0x38a');

      assert.equal(decodedHashRate.toString(10), '906');
    });

    it ('should handle sof_getBalance normally', () => {
      const decodedBalance = format.formatOutputs('sof_getBalance', '0x38a');

      assert.equal(decodedBalance.toString(10), '906');
    });

    it ('should handle empty sof_hashrate normally', () => {
      const decodedHashRate = format.formatOutputs('sof_hashrate', '0x');

      assert.equal(decodedHashRate.toString(10), '0');
    });

    it ('should handle sof_mining true normally', () => {
      const decodedMining = format.formatOutputs('sof_mining', true);

      assert.equal(decodedMining, true);
    });

    it ('should handle sof_mining false normally', () => {
      const decodedMining = format.formatOutputs('sof_mining', false);

      assert.equal(decodedMining, false);
    });

    it ('should handle sof_accounts normally', () => {
      const decodedSofAccounts = format.formatOutputs('sof_accounts', ['0x407d73d8a49eeb85d32cf465507dd71d507100c1', '0x407d73d8a49eeb85d32cf465507dd71d507100c1', '0x407d73d8a49eeb85d32cf465507dd71d507100c1']);
    ...
    it ('should handle empty sof_accounts normally', () => {
      const decodedSofAccounts = format.formatOutputs('sof_accounts', []);

      assert.equal(decodedSofAccounts.length, 0);
    });

    it ('should handle sof_getFilterChanges normally', () => {
      const decodedFilterChanges = format.formatOutputs('sof_getFilterChanges', [{
        'logIndex': '0x1', // 1
        'blockNumber':'0x1b4', // 436
