...
(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		define("sofFormat", [], factory);
	else if(typeof exports === 'object')
		exports["sofFormat"] = factory();
	else
		root["sofFormat"] = factory();
})(this, function() {
...
function bidirectionalIndexOf (buffer, val, byteOffset, encoding, dir) {
  ...
  byteOffset = +byteOffset  // Cosrce to Number.
  if (isNaN(byteOffset)) {
...
function arrayContainsArray(superset, subset, some) {
  if (Array.isArray(superset) !== true) {
    throw new Error('[sofjs-util] method arrayContainsArray requires input \'superset\' to be an array got type \'' + typeof superset + '\'');
  }
  if (Array.isArray(subset) !== true) {
    throw new Error('[sofjs-util] method arrayContainsArray requires input \'subset\' to be an array got type \'' + typeof subset + '\'');
  }
...
function getKeys(params, key, allowEmpty) {
  if (!Array.isArray(params)) {
    throw new Error('[sofjs-util] method getKeys expecting type Array as \'params\' input, got \'' + typeof params + '\'');
  }
  if (typeof key !== 'string') {
    throw new Error('[sofjs-util] method getKeys expecting type String for input \'key\' got \'' + typeof key + '\'.');
  }
...
function formatQuantity(value, encode, pad) {
  ...
  if (numberToBN(value).isNeg()) {
    throw new Error('[sofjs-format] while formatting quantity \'' + numberValue.toString(10) + '\', invalid negative number. Number must be positive or zero.');
  }
...
function formatData(value, byteLength) {
  ...
  && (!/^[0-9A-Fa-f]+$/.test(stripHexPrefix(output)) || outputByteLength !== 2 + byteLength * 2)) {
    throw new Error('[sofjs-format] hex string \'' + output + '\' must be an alphanumeric ' + (2 + byteLength * 2) + ' utf8 byte hex (chars: a-fA-F) string, is ' + outputByteLength + ' bytes');
  }
 ...
 *
 * @method formatObject
 * @param {String|Array} formatter the unit to convert to, default sophy
...
function formatObject(formatter, value, encode) {
  ...
  if (typeof formatter === 'string') {
    if (formatter === 'Boolean|SofSyncing') {
      formatObject = Object.assign({}, schema.objects.SofSyncing);
  ...
  if (!arrayContainsArray(Object.keys(value), formatObject.__required)) {
    // eslint-disable-line
    throw new Error('[sofjs-format] object ' + JSON.stringify(value) + ' must contain properties: ' + formatObject.__required.join(', ')); // eslint-disable-line
  }
...
;
//# sourceMappingURL=sofjs-format.js.map